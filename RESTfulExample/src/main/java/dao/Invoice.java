...
public class Invoice {
	...
	+"</tr></tbody></table>");
	 ...
	 /**
		 * Generate email to eazelyf support for order query
		 * @param user
		 * @param order
		 * @param orderItemList
		 * @return
		 */
		 public static Boolean generateQueryEmail(User user) { 
			...
			try {
				generateMailMessage.setFrom(new InternetAddress("somnathdutta048@gmail.com"));
				generateMailMessage.addRecipient(Message.RecipientType.TO,new InternetAddress(user.getEmailId()) );
				generateMailMessage.setSubject("Query From Customer");
				String emailBody = user.getMyMessage();
				generateMailMessage.setContent(emailBody, "text/html");
				System.out.println("Mail Session has been created successfully..");
				// Step3
				System.out.println("\n 4th ===> Get Session and Send mail");
